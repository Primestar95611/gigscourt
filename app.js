...
vapidKey:
"BJRlkPoQeveLqWzxp3CxzpkO4__sXqaCaA8loG9KCpN0z7rlh8aYr3d_tav7LB0Ra3LG50m0EUyVokh66p_9TO4",
...
toast.style.cssText =
'position:fixed;bottom:100px;left:20px;right:20px;background:#4CAF50;color:white;padding:12px;text-align:center;border-radius:8px;z-index:9999;';
...
<input type="text" id="complete-business"
...
onclick="toggleService('Barber')">Barber</button>

<button type="button" class="service-pill"
onclick="toggleService('Tech')">Tech</button>

<button type="button" class="service-pill"
onclick="toggleService('Design')">Design</button>

<button type="button" class="service-pill"
onclick="toggleService('Marketing')">Marketing</button>
...
<textarea id="complete-bio" rows="3" placeholder="Tell customers
about yourself"></textarea>

</div>

<button class="btn btn-full" onclick="saveProfile()">Save
Profile</button>
...
const businessName =
document.getElementById('complete-business').value;
...
await
firebase.firestore().collection('users').doc(firebase.auth().currentUser.uid).set(updateData,
{ merge: true });
...
function loadHomeTab(noAutoRefresh = false) {
...
<button id="enable-notify-btn" class="btn-small"
style="background:#8B0000; color:white; border-radius:20px; padding:5px
12px; margin-right:8px;">🔔 Enable</button>
...
<div id="load-more-trigger" style="height: 20px; margin: 20px;
text-align: center;">

<div id="load-more-spinner" class="loading-spinner hidden"
style="padding: 10px;">
...
<div id="load-more-end" class="hidden" style="color:
var(--text-secondary); font-size: 12px;">You've seen all
providers</div>
...
if (!noAutoRefresh) {
    ...
    // Show cached providers if they exist
...
newProviders.sort((a, b) => parseFloat(a.distance) -
...
grid.innerHTML = '<div class="error-state">Failed to load
providers. Pull down to refresh.</div>';
...
alt="${escapeHtml(provider.businessName)}" loading="lazy">
...
<h3
class="provider-name">${escapeHtml(provider.businessName)}</h3>

<div class="provider-rating">

<span class="stars">⭐ ${provider.rating || '0.0'}</span>

<span class="review-count">(${provider.reviewCount ||
0})</span>

</div>

<div
class="provider-services">${escapeHtml(displayServices)}${hasMoreServices}</div>

<div class="provider-distance" onclick="event.stopPropagation();
showOnMap('${provider.id}')">
...
<img src="${provider.profileImage ? provider.profileImage +
'?tr=w-100,h-100,format-webp' : 'https://via.placeholder.com/100'}"
...
<div class="quick-view-rating">⭐ ${provider.rating || '0.0'}
(${provider.reviewCount || 0} reviews)</div>

<div class="quick-view-distance">📍 ${provider.distance} km
away</div>
...
${(provider.services || []).map(s => `<span
class="service-tag">${s}</span>`).join('')}

</div>

<div class="quick-view-bio">

${provider.bio ? provider.bio.substring(0, 100) + (provider.bio.length
> 100 ? '...' : '') : 'No bio yet.'}
...
<button class="btn" onclick="viewProfile('${provider.id}',
'${window.currentTab || 'home'}')">View Profile</button>

<button class="btn" onclick="messageUser('${provider.id}',
'${window.currentTab || 'home'}')">Message</button>

<button class="btn"
onclick="getDirectionsToProvider('${provider.id}',
'quickview')">Directions</button>
...
<button class="btn btn-small" onclick="markAllRead()">Mark all
read</button>
...
showToast('⚠️ You already have a pending job with this client. Complete
it first.');
...
const workerUrl =
'https://gigscourtnotification.agboghidiaugust.workers.dev';
...
body: `${providerData.businessName} wants to work with you. Please
review.`,
...
document.querySelectorAll('.gig-confirmation').forEach(el =>
el.remove());
...
let scrollableContainer = document.querySelector('.home-container,
.profile-container, .search-container, .chat-messages');
...
<button class="close-btn" onclick="(function(b){let
c=b.closest('.review-modal'); if(c)c.remove(); let
s=document.querySelector('.home-container, .profile-container,
.search-container, .chat-messages');
if(s)s.style.overflow='';})(this)">✕</button>
...
placeholder="Write your review (required)" rows="4"></textarea>

<button class="btn submit-review-btn"
onclick="submitReview('${providerId}', '${jobId}')">Submit
Review</button>
...
const providerRef =
firebase.firestore().collection('users').doc(providerId);
...
const oldTotal = (provider.rating || 0) * (provider.reviewCount ||
0);
...
showToast('⚠️ Provider has insufficient credits. Job cannot be
completed.');
...
body: `${clientName} left you a ${rating}★ review! This will boost
your profile.`,
...
const otherUserImage = chat.userImages?.[otherUserId] ||
'https://via.placeholder.com/40';

clientsHtml += `

<div class="client-item" onclick="selectClient('${otherUserId}',
'${otherUserName}')">
...
onclick="this.closest('.register-job-modal').remove()">✕</button>
...
<p class="points-info">This will cost 3 points (You have
${currentUserData?.points || 0} points)</p>

<div class="clients-list">

${clientsHtml || '<div class="no-clients">No recent chats. Start
a conversation first.</div>'}
...
if (!confirm(`Register gig with ${clientName} for 3 points?`))
return;
...
const providerDoc = await
firebase.firestore().collection('users').doc(providerId).get();
...
const date = review.updatedAt?.toDate().toLocaleDateString() ||
'Unknown';
...
<img src="${review.clientProfileImage ||
'https://via.placeholder.com/40'}" class="review-profile-img">

<div class="review-business-name">${review.clientBusinessName ||
'Anonymous'}</div>
...
<div class="review-meta">${review.jobsTogether} completed gigs
together • Updated ${date}</div>
...
<button class="close-btn"
onclick="(function(b){b.closest('.reviews-modal').remove();
document.body.style.overflow = '';})(this)">✕</button>
...
<div class="stars-big">${generateStarString(provider.rating ||
0)}</div>

<div class="review-count-big">${provider.reviewCount || 0}
reviews</div>
...
window.getDirectionsToProvider = async function(providerId, fromScreen =
'profile') {
...
document.querySelector('.directions-toggle-btn');
...
<path d="M12 3L3 9L5 9V19H9V15H15V19H19V9L21 9L12 3Z"
fill="currentColor"/>
...
<path d="M4 4H20C21.1 4 22 4.9 22 6V18C22 19.1 21.1 20 20 20H4C2.9 20
2 19.1 2 18V6C2 4.9 2.9 4 4 4Z" stroke="currentColor"
...
<span id="messages-tab-badge" class="tab-badge hidden">0</span>
...
12 2zm0 4c1.93 0 3.5 1.57 3.5 3.5S13.93 13 12 13s-3.5-1.57-3.5-3.5S10.07
6 12 6zm0 14c-2.03 0-4.43-.82-6-2.28 0-2.56 4-4 6-4s6 1.44 6 4c-1.57
1.46-3.97 2.28-6 2.28z" fill="currentColor"/>
...
const profileDoc = await
firebase.firestore().collection('users').doc(targetUserId).get();

if (!profileDoc.exists) {

container.innerHTML = '<div class="error-state">Profile not
found</div>';
...
savesCount = Object.keys(currentUserDoc.data()?.savedProfiles ||
{}).length;
...
<div class="profile-sticky-header" style="display: flex;
justify-content: space-between; align-items: center; padding: 16px 16px
12px 16px;">

<button class="profile-back-btn" onclick="goBack()"
style="background: none; border: none; font-size: 28px; cursor:
pointer; padding: 0;">←</button>

<span style="font-size: 18px; font-weight: 600; flex: 1; text-align:
center;">${profile.businessName || 'Profile'}</span>

${isOwnProfile ? `<button class="history-btn"
onclick="openJobHistory()" style="background: none; border: none;
font-size: 20px; cursor: pointer; padding: 0;">⏱️</button>` :
...
<button class="history-btn" onclick="openJobHistory()"
style="background: none; border: none; font-size: 20px; cursor:
pointer; padding: 0;">⏱️</button>

</div>

` : ''}

<div class="profile-stats-row" style="${hideTabBar ? 'margin-top:
0;' : ''}">

<div class="profile-picture">

<img src="${profile.profileImage ? profile.profileImage +
'?tr=w-80,h-80,format-webp' : 'https://via.placeholder.com/80'}"
alt="${profile.businessName}">

${isOwnProfile ? '<div class="camera-icon"
onclick="openImageUpload()">📷</div>' : ''}
...
class="stat-number">${profile.jobsDone || 0}</span><span
class="stat-label">Gigs</span></div>

<div class="stat-item clickable"
onclick="showProviderReviews('${profile.id}')"><span
class="stat-number">${profile.rating || 0}</span><span
class="stat-label">★ Rating</span></div>

${isOwnProfile ? `<div class="stat-item clickable"
onclick="openSavedModal()"><span
class="stat-number">${savedCount}</span><span
class="stat-label">Saved</span></div>` : ''}

<div class="stat-item ${isOwnProfile ? 'clickable' : ''}"
onclick="${isOwnProfile ? 'openSavesModal()' : ''}"><span
class="stat-number">${savesCount}</span><span
class="stat-label">Saves</span></div>
...
${isOwnProfile ? `<div class="profile-actions-header"><div
class="points-display" style="text-align:center; margin-bottom:8px;
font-size:12px; color:#FFD700; font-weight:500;">💰 You have
${profile.points || 0} credits remaining</div><button
class="register-job-btn" onclick="showRegisterJobModal()">Register
Gig (3 pts)</button></div>` : ''}

<div class="profile-meta">Joined ${profile.createdAt ? new
Date(profile.createdAt.toDate()).toLocaleDateString('en-US', { month:
'short', year: 'numeric' }) : 'Unknown'} •
${profile.jobsThisMonth || 0} gigs this month</div>
...
<div class="profile-bio">${profile.bio || 'No bio
yet.'}</div>

<div class="contact-now-section"
id="contact-now-section-${profile.id}">

${!isOwnProfile && profile.phoneNumber ? `

<button class="contact-now-btn"
id="contact-now-btn-${profile.id}">📞 Contact Now</button>

<div id="phone-reveal-${profile.id}" style="display:none;"
class="contact-phone-section">
...
onclick="window.location.href='tel:${profile.phoneNumber}'">Call
Now</button>
...
${profile.locationGeo ? `<div class="profile-contact
${!isOwnProfile ? 'clickable-location' : ''}" ${!isOwnProfile ?
`onclick="getDirectionsToProvider('${profile.id}')"` :
''}><span class="contact-icon">📍</span><span
class="contact-text">${profile.locationDescription ||
`${profile.locationGeo.latitude},
${profile.locationGeo.longitude}`}</span></div>` : ''}

<div class="profile-section"><h3
class="section-title">Services</h3><div
class="services-horizontal">${(profile.services ||
[]).map(service => `<span
class="service-pill-static">${service}</span>`).join('')}${(profile.pendingServices
|| []).map(service => `<span class="service-pill-static
pending">${service} (pending)</span>`).join('')}</div></div>

${!isOwnProfile ? `<div class="profile-section"><div
class="section-header"><h3
class="section-title">Contact</h3></div><div
class="profile-contact"><span
class="contact-icon">✉️</span><span
class="contact-text">Message in app</span></div></div>` :
''}

<div class="profile-actions">${isOwnProfile ? `<button
class="btn" onclick="openEditProfile()">Edit
Profile</button><button class="btn btn-outline"
onclick="shareProfile()">Share</button>` : `<button
class="btn" onclick="messageUser('${profile.id}',
'profile')">Message</button><button class="btn"
onclick="toggleSaveProfile('${profile.id}')"
id="save-btn-${profile.id}">Save</button><button class="btn
btn-outline"
onclick="shareProfile('${profile.id}')">Share</button>`}</div>

<div class="profile-section"><div class="section-header"><h3
class="section-title">Portfolio ${profile.portfolioImages?.length ?
`(${profile.portfolioImages.length})` : ''}</h3>${isOwnProfile ?
'<button class="btn-small" onclick="addPortfolioImages()">+
Add</button>' : ''}</div><div
class="portfolio-grid">${(profile.portfolioImages ||
[]).map((img, index) => `<div class="portfolio-item"
onclick="openPhotoSwipe(${index})"><img
src="${img}?tr=w-150,h-150,format-webp"
loading="lazy">${isOwnProfile ? '<div class="delete-overlay"
onclick="deleteImage(event, \'' + img + '\')">✕</div>' :
''}</div>`).join('')}${!profile.portfolioImages?.length ? '<p
class="empty-portfolio">📸 Post pictures so people can see what you
are capable of.</p>' : ''}</div></div>

${isOwnProfile && (!profile.portfolioImages?.length) ? '<div
class="profile-section"><p class="hint" style="text-align:center;
color:var(--text-secondary);">📸 Add portfolio images to attract more
clients</p></div>' : ''}

${isOwnProfile && (profile.points || 0) <= 3 ? '<div
class="profile-section"><p class="hint" style="text-align:center;
color:#ff9800;">⚠️ Low on credits. Buy more to register
jobs.</p></div>' : ''}
...
const savedScroll =
sessionStorage.getItem(`profile_scroll_${targetUserId}`);
...
sessionStorage.setItem(`profile_scroll_${targetUserId}`,
container.scrollTop);
...
container.innerHTML = '<div class="error-state">Error loading
profile</div>';
...
const contactBtn =
document.getElementById(`contact-now-btn-${providerId}`);

const phoneReveal =
document.getElementById(`phone-reveal-${providerId}`);

const contactSection =
document.getElementById(`contact-now-section-${providerId}`);
...
const hasPending = await checkPendingJobLimit(providerId,
currentUserId);
...
body: `${clientName} viewed your contact. Reminder to register job if
you worked together.`,
...
const date = job.reviewedAt?.toDate().toLocaleDateString() ||
'Unknown';
...
<div class="review-business-name">Provider:
${job.providerId}</div>
...
jobsHtml = '<div class="no-reviews">No completed jobs
yet</div>';
...
<button class="close-btn"
onclick="this.closest('.reviews-modal').remove()">✕</button>
...
${hideTabBar ? '<button class="back-btn" onclick="goBack()"
...
<h1 class="profile-business-name">${profile.businessName ||
'Business Name'}</h1>
...
<div class="stat-item clickable"
onclick="showProviderReviews('${profile.id}')">
...
<div class="stat-item ${isOwnProfile ? 'clickable' : ''}"
onclick="${isOwnProfile ? 'openSavesModal()' : ''}">
...
onclick="showRegisterJobModal()">Register Gig (3 pts)</button>
...
<span class="contact-text">${profile.locationDescription ||
`${profile.locationGeo.latitude},
${profile.locationGeo.longitude}`}</span>
...
`<span class="service-pill-static pending">${service}
(pending)</span>`
...
<button class="btn" onclick="openEditProfile()">Edit
Profile</button>

<button class="btn btn-outline"
onclick="shareProfile()">Share</button>

` : `

<button class="btn" onclick="messageUser('${profile.id}',
'profile')">Message</button>

<button class="btn" onclick="toggleSaveProfile('${profile.id}')"
id="save-btn-${profile.id}">Save</button>

<button class="btn btn-outline"
onclick="shareProfile('${profile.id}')">Share</button>
...
`(${profile.portfolioImages.length})` : ''}</h3>

${isOwnProfile ? '<button class="btn-small"
onclick="addPortfolioImages()">+ Add</button>' : ''}
...
${isOwnProfile ? '<div class="delete-overlay"
onclick="deleteImage(event, \'' + img + '\')">✕</div>' :
''}
...
${!profile.portfolioImages?.length ? '<p
class="empty-portfolio">📸 Post pictures so people can see what you
are capable of.</p>' : ''}
...
${isOwnProfile && (!profile.portfolioImages?.length) ? '<div
class="profile-section"><p class="hint" style="text-align:center;
color:var(--text-secondary);">⭐ More completed jobs equals more
clients</p></div>' : ''}
...
${isOwnProfile && (profile.jobsDone || 0) === 0 ? '<div
class="profile-section"><p class="hint" style="text-align:center;
color:var(--text-secondary);">🚀 Complete your first job to build
your reputation</p></div>' : ''}
...
const targetUserDoc = await
firebase.firestore().collection('users').doc(profileId).get();
...
onclick="viewProfileFromModal('${userId}')">

<img src="${userData.profileImage ? userData.profileImage +
'?tr=w-50,h-50' : 'https://via.placeholder.com/50'}"
class="modal-item-image">

<div class="modal-item-info">

<div class="modal-item-name">${userData.businessName ||
'Business Name'}</div>

<div class="modal-item-rating">⭐ ${userData.rating ||
'0.0'}</div>

</div>

<button class="modal-unsave-btn" onclick="unsaveProfile(event,
'${userId}')">Unsave</button>
...
<div class="modal-item"
onclick="viewProfileFromModal('${doc.id}')">
...
fetch('https://gigscourt.vercel.app/api/imagekit-auth');
...
const updatedImages = (userData.portfolioImages || []).filter(url
=> url !== imageUrl);
...
<button class="back-btn" onclick="loadProfileTab(null,
false)">←</button>
...
currentUserData.profileImage + '?tr=w-80,h-80' :
'https://via.placeholder.com/80'}" alt="Profile">

<div class="change-picture-btn"
onclick="openImageUpload()">Change</div>
...
<input type="text" id="edit-business-name"
value="${currentUserData?.businessName || ''}">
...
<textarea id="edit-bio" rows="3">${currentUserData?.bio ||
''}</textarea>
...
📍 ${currentUserData?.locationDescription ||
(currentUserData?.locationGeo ?
`${currentUserData.locationGeo.latitude},
${currentUserData.locationGeo.longitude}` : 'Set your location')}
...
onclick="removeService('${service}')">✕</span></span>`
...
<button class="btn btn-outline" onclick="logout()">Log
Out</button>

<button class="btn btn-outline delete-account"
...
await
firebase.firestore().collection('users').doc(firebase.auth().currentUser.uid).update(updateData);
...
class="location-search-input" placeholder="Search for a place...">
...
<input type="text" id="location-address" class="location-address"
readonly placeholder="Drag the map to set location">
...
<textarea id="location-description" rows="2" placeholder="e.g.,
Beside the blue church, after the mechanic
village">${currentUserData?.locationDescription ||
''}</textarea>
...
const geopoint = new firebase.firestore.GeoPoint(selectedLocation.lat,
...
const newService =
document.getElementById('new-service').value.trim();
...
if (['Barber', 'Tech', 'Design',
'Marketing'].includes(newService)) {
...
const profileDoc = await
firebase.firestore().collection('users').doc(targetId).get();
...
const bio = profile.bio ? profile.bio.substring(0, 100) : 'Check out my
profile on GigsCourt';
...
if (window.currentPortfolioImages &&
window.currentPortfolioImages.length > 0) {
...
function loadSearchTab(noAutoRefresh = false) {
...
<span class="radius-value" id="radius-value">${currentRadius}
km</span>

<input type="range" id="radius-slider" class="radius-slider"
min="1" max="200" value="${currentRadius}" step="1">

</div>

<div id="search-hint" style="font-size:12px;
color:var(--text-secondary); text-align:center; margin-top:5px;">⭐
Top providers have completed jobs and reviews</div>
...
getUserLocation(!noAutoRefresh);

setupSearchListeners();

}

function getUserLocation(shouldAutoRefresh = true) {
...
listContainer.innerHTML = '<div class="empty-list">🔍 Be the first
to offer this service in this location</div>';
...
searchProviders.sort((a, b) => parseFloat(a.distance) -
parseFloat(b.distance));
...
<div class="rating-badge">⭐ ${provider.rating ||
'0.0'}</div>
...
⭐ ${provider.rating || '0.0'} (${provider.reviewCount ||
0})<br>

📍 ${provider.distance} km<br>

<button class="popup-btn"
onclick="viewProviderFromMap('${provider.id}')">View</button>
...
onclick="openQuickViewFromSearch('${provider.id}')">

<img src="${provider.profileImage ? provider.profileImage +
'?tr=w-40,h-40,format-webp' : 'https://via.placeholder.com/40'}"
...
if (providerDrawer.scrollTop + providerDrawer.clientHeight >=
providerDrawer.scrollHeight - 100) {
...
const name =
item.querySelector('.list-item-name')?.textContent.toLowerCase() ||
'';
...
L.latLng(provider.location?.lat || userLocation.lat,
provider.location?.lng || userLocation.lng)
...
const hasPending = await checkPendingJobLimit(currentUserId,
otherUserId);
...
document.getElementById('reminder-banner-container');

const reminderText = document.getElementById('reminder-text');

if (bannerContainer && reminderText) {

reminderText.innerHTML = `✏️ Did you offer services to this person?
<button onclick="window.registerJob('${otherUserId}')"
style="background:#8B0000; color:white; border:none; padding:2px 8px;
border-radius:12px; margin-left:8px;">Register Now</button>`;
...
!document.getElementById('review-button-container')) {
...
<button class="btn confirm-gig-btn"
onclick="confirmGig('${jobId}', '${otherUserId}')">Accept &
Review</button>
...
<button class="btn" onclick="switchTab('search')">Find
Providers</button>
...
// Only fetch once on initial load, real-time listener will handle updates
fetchConversations();
...
html += `<div
class="chat-date-separator">${msg.timestamp?.toDate().toLocaleDateString()}</div>`;
...
messagesContainer.insertBefore(existingReviewBtn,
...
formatMessageTime(chat.lastMessageTimestamp.toDate()) : '';

const unread = chat.lastMessageSender !== currentUserId &&
...
<div id="chat-${chat.id}" class="conversation-item ${unread ?
'unread' : ''}">
...
<span class="preview-text">${lastMessage || 'No messages
yet'}</span>

${statusIcon ? `<span class="message-status ${chat.lastMessageRead
? 'read' : ''}">${statusIcon}</span>` : ''}
...
} else if (previousScreen === 'home' || previousScreen ===
'search') {
...
onclick="${backAction}">←</button>

<img src="${otherUserImage}" class="chat-header-image"
onclick="viewProfileFromChat('${otherUserId}')"
style="cursor:pointer;">

<span class="chat-header-name"
onclick="viewProfileFromChat('${otherUserId}')"
style="cursor:pointer;">${otherUserName}</span>

<button id="register-job-chat-btn" class="btn-small"
style="background:#8B0000; color:white; border-radius:20px; padding:5px
12px; margin-left:auto;">📋 Register Job</button>
...
<button id="dismiss-reminder" style="margin-left:10px;
...
const time = msg.timestamp?.toDate().toLocaleTimeString([], { hour:
...
${isMine ? `<span class="message-status ${msg.read ? 'read' :
''}">${status}</span>` : ''}
...
const otherUserId = chatData.data()?.participants.find(id => id !==
currentUserId);
...
const recipientId = chatData.participants.find(id => id !==
currentUserId);

const recipientDoc = await
...
function loadMessagesTab(noAutoRefresh = false) {
...
if (!noAutoRefresh) {
    ...
    // Show cached conversations if they exist
...
window.switchTab = (tab) => {
    const now = Date.now();
    const lastTap = lastTabTap[tab] || 0;
    const isDoubleTap = (now - lastTap) < 1500;
    
    // Update last tap time
    lastTabTap[tab] = now;
    
    // Check if this is the active tab
    const isActiveTab = (window.currentTab === tab);
    
    if (isActiveTab && isDoubleTap) {
        // Double-tap on active tab - trigger refresh
        if (tab === 'home') {
            refreshProviders();
        } else if (tab === 'search') {
            loadNearbyProviders(true);
        }
        // Messages double-tap does nothing (real-time handles)
        // Profile double-tap does nothing
        return;
    }
    ...
    if (isActiveTab && !isDoubleTap) {
        ...
        if (tab === 'home') {
            const scrollable = document.querySelector('.home-scrollable');
            if (scrollable) scrollable.scrollTop = 0;
        } else if (tab === 'search') {
            const scrollable = document.querySelector('.provider-drawer');
            if (scrollable) scrollable.scrollTop = 0;
        } else if (tab === 'messages') {
            const scrollable = document.querySelector('.messages-scrollable');
            if (scrollable) scrollable.scrollTop = 0;
        }
        return;
    }
    
    // Not active tab - switch to it (no auto-refresh)
    
    // Clean up previous tab resources
    if (window.currentTab === 'home') {
        cleanupInfiniteScroll();
    }
    if (window.currentTab === 'messages') {
    ...
    // Load the tab without auto-refresh
...
function attachMessagesListener() {
    if (conversationsUnsubscribe) return;
    ...
    conversationsUnsubscribe = firebase.firestore()
        ...
        .onSnapshot((snapshot) => {
            ...
            snapshot.forEach(doc => {
                ...
                // Count unread conversations where last message not read and not sent by current user
                const isUnread = chat.lastMessageSender !== userId && !chat.lastMessageRead;
                if (isUnread) {
                    unreadCount++;
                }
...
async function markConversationAsRead(chatId) {
    ...
    try {
        ...
        // Update unread count
        const unreadCount = Math.max(0, unreadConversationsCount - 1);
        updateMessagesBadge(unreadCount);
    } catch (error) {
...
<p class="small">Click the link in the email to verify your
account</p>

<button class="btn" onclick="checkVerification()">I've
verified</button>

<button class="btn btn-outline"
onclick="resendVerification()">Resend email</button>

<button class="btn btn-outline"
onclick="firebase.auth().signOut()">Back to login</button>
...
<button class="btn btn-outline btn-full" onclick="alert('Phone
signup coming soon!')">
...
<input type="text" id="signup-business" required placeholder="Your
business name">
...
<input type="text" id="custom-services" placeholder="e.g.,
...
const customList = customServices.split(',').map(s =>
s.trim()).filter(s => s);
...
services: services.filter(s => ['Barber', 'Tech', 'Design',
'Marketing'].includes(s)),

pendingServices: services.filter(s => !['Barber', 'Tech',
'Design', 'Marketing'].includes(s)),
...
if (confirm('Are you sure you want to delete your account? This cannot
be undone.')) {
...
<button class="btn" onclick="switchTab('home')">Go
Home</button>
...
<button class="admin-subtab ${adminCurrentTab === 'dashboard' ?
'active' : ''}" onclick="switchAdminTab('dashboard')">

📊 Dashboard

</button>

<button class="admin-subtab ${adminCurrentTab === 'users' ?
'active' : ''}" onclick="switchAdminTab('users')">

👥 Users

</button>

<button class="admin-subtab ${adminCurrentTab === 'approvals' ?
'active' : ''}" onclick="switchAdminTab('approvals')">
...
container.innerHTML = '<div class="admin-loading"><div
class="spinner"></div></div>';

try {

let statsDoc = await
firebase.firestore().collection('stats').doc('dashboard').get();
...
return (b.data().createdAt?.toDate() || 0) -
...
statsDoc = await
firebase.firestore().collection('stats').doc('dashboard').set({
...
<img src="${item.profileImage ? item.profileImage + '?tr=w-40,h-40'
: 'https://via.placeholder.com/40'}" class="recent-image">

<div class="recent-info">

<div class="recent-name">${item.businessName ||
'Unnamed'}</div>
...
<img src="${user.profileImage ? user.profileImage + '?tr=w-50,h-50'
: 'https://via.placeholder.com/50'}" class="user-item-image">

<div class="user-item-info">

<div class="user-item-header">

<span class="user-item-name">${user.businessName ||
'Unnamed'}</span>
...
<div class="user-item-email">${user.email || 'No
email'}</div>
...
container.innerHTML = '<div class="empty-approvals">No pending
approvals</div>';
...
<img src="${user.profileImage ? user.profileImage + '?tr=w-40,h-40'
: 'https://via.placeholder.com/40'}" class="approval-image">

<div class="approval-user">

<div class="approval-name">${user.businessName ||
'Unnamed'}</div>
...
id="service-${user.id}-${service.replace(/\s+/g, '-')}">
...
onclick="approveService('${user.id}', '${service.replace(/'/g,
"\\'")}')">Approve</button>

<button class="btn-small edit-btn"
onclick="editService('${user.id}', '${service.replace(/'/g,
"\\'")}')">Edit</button>
...
const updatedPending = userData.pendingServices.filter(s => s !==
service);
...
console.log('Failed to send service approval notification:',
notifyError);

}

await sendAdminNotification(userId, `Your service "${service}" has
been approved!`);

const serviceElement =
document.getElementById(`service-${userId}-${service.replace(/\s+/g,
'-')}`);
...
if (approvalItem &&
!approvalItem.querySelector('.pending-service-item')) {
...
!approvalsContainer.querySelector('.approval-item')) {

document.getElementById('admin-content').innerHTML = '<div
class="empty-approvals">No pending approvals</div>';
...
const updatedPending = userData.pendingServices.map(s => s === service
? newService : s);
...
approveBtn.setAttribute('onclick', `approveService('${userId}',
'${newService.replace(/'/g, "\\'")}')`);

}

if (editBtn) {

editBtn.setAttribute('onclick', `editService('${userId}',
'${newService.replace(/'/g, "\\'")}')`);
...
await
firebase.firestore().collection('chats').doc(existingChatId).update({
...
html += '<div class="profile-section"><p class="hint"
style="text-align:center; color:var(--text-secondary);">🚀 Complete
your first job to build your reputation</p></div>';

}

// Message for users who have done at least one job but last job was >7
days ago
...
html += '<div class="profile-section"><p class="hint"
style="text-align:center; color:var(--text-secondary);">⭐ More
completed jobs equals more clients</p></div>';
